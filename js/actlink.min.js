			...
			if(response){
				...
				$.each( response, function(key,value) {
					...
					'" data-owner="'+owner+'" data-group="'+group+'" data-bktarget="'+value.bktarget+'" data-path="'+path+'">'+
						'<a href="'+uri+'"><i class="icon icon-folder deic_green"></i><span>'+value.bktitle+
						'<span class="icon-cancel-circled delete_bookmark" style="display:none"></span></span></a></li>';
				});
				$('.nav-sidebar li[data-id^="internal-bookmarks"]').remove();
				$('.sidebar ul.nav-sidebar li[data-id="files_index"]').after(bookmarks);
				if($('.sidebar ul.nav-sidebar li#places span i.icon-angle-right').is(':visible')){
					$('.sidebar ul.nav-sidebar li[data-id^="internal-bookmarks"]').hide();
				}
				$('.nav-sidebar li[data-id^="internal-bookmarks"]').on(
						{mouseenter: function(){
					$(this).find('.delete_bookmark').show();
			  }, mouseleave: function(){
					$(this).find('.delete_bookmark').hide();
			  }, click: function(e){
			    var target = $(e.target);
			    if(target.is( ".delete_bookmark" )){
						e.stopPropagation();
						e.preventDefault();
						setBookmark($(this).attr('data-bktarget'));
			    }
			  }});
			}
		}
...
$(document).ready(function(){
	...
	$('.sidebar ul.nav-sidebar').sortable({
		helper: 'clone',
		update: function(event, ui){
			var a=new Array();
			$(".sidebar ul.nav-sidebar").find('li[data-id^="internal-bookmarks"]').each(function(c,b){
	...
	$('.sidebar ul.nav-sidebar').disableSelection();
	...
	if(typeof OCA.Files!=="undefined"){
		...
		function(d, context){
		  ...
		  var id = $tr.data('id');
		  // This is to avoid the REPORT query of iOS returning URLs of the form
		  // /remote.php/dav/files/some_user/group/...
		  // URLs of that form are reserved for bookmarks pointing to group folders.
		  // Bookmarks of the form  /remote.php/dav/files/some_user/sharingin/...
		  // are reserved for bookmarks pointing to folders shared with me.
		  if($tr.data('path')=="/sharingin" || $tr.data('path')=="/group" ||
				$tr.data('path').indexOf("/sharingin/")==0 || $tr.data('path').indexOf("/group/")==0 ||
				$tr.data('file')=="sharingin" || $tr.data('file')=="group"){
					OC.dialogs.alert(t('internal_bookmarks', 'Error setting bookmark: name is reserved'), t('internal_bookmarks', 'Error'));
				}
				else{
  ...
  $('.sidebar ul.nav-sidebar').on('click', 'li[data-id^="internal-bookmarks"]', function(e) {
		$('.sidebar ul.nav-sidebar').find('.active').removeClass('active');	
		...
		e.stopPropagation();
		e.preventDefault();
		if(typeof OCA.Files != 'undefined' && OCA.Files.App != 'undefined' && $('#app-content-files').length){
			var view;
			if($(this).attr('data-group')){
				var group = $(this).attr('data-group').replace( /(:|\.|\[|\]|,|=)/g, "\\$1" );
				view = 'user-groups_'+group;
				OCA.Files.App.setActiveView(view, {silent: true});
			  OCA.UserGroups.App.initGroupFileList($('div[id="app-content-user-groups_'+group+'"]')[0]);
			}
			else{
				view = 'files';
				OCA.Files.App.setActiveView(view, {silent: true});
			...
			OCA.Files.App.fileList.changeDirectory($(this).attr('data-path'), true, true,
					...
					$(this).attr('data-storage'),
					// callback
					function(path, id, owner, group){
						OC.dialogs.confirm(t('internal_bookmarks', 'This bookmark points to a folder that no longer exists')+': '+
								OCA.intBookmarks.App.bktarget+'. '+t('internal_bookmarks', 'Would you like to delete it?'),
										t('internal_bookmarks', 'Confirm delete'),
						function(res){
							if(res){
								setBookmark(OCA.intBookmarks.App.bktarget);
							}
						},
						false
						);
			...
			window.location.href = "/index.php/apps/files?dir="+$(this).attr('data-path')+
			"&bookmark_id="+$(this).attr('data-id')+
			"&id="+$(this).attr('data-file_id')+
			"&owner="+$(this).attr('data-owner')+
			($(this).attr('data-group')?"&group="+$(this).attr('data-group')+"&view=user-groups_"+$(this).attr('data-group').replace( /(:|\.|\[|\]|,|=)/g, "\\$1" ):($(this).attr('data-file_id')?'':'&view=bookmark'));
		}
	});
